...
class Tools
{
    ...
    public static function getValue($key) { return isset(self::$params[$key]) ? self::$params[$key] : false; }
    public static function str2url($value) { return $value; }
    public static function strReplaceFirst($search, $replace, $value) { return preg_replace('/'.preg_quote($search, '/').'/', $replace, $value, 1); }
    public static function url($url, $query) { return $url.(strpos($url, '?') === false ? '?' : '&').$query; }
}
class Configuration
{
    public static $rewriting = 1;
    public static function get($name) { return $name === 'PS_PRODUCTS_PER_PAGE' ? 10 : self::$rewriting; }
}
class Validate
{
    public static function isLoadedObject($object) { return !empty($object->id); }
}
class Context
{
    public static $current;
    public static function getContext() { return self::$current; }
}
class FixtureObject
{
    ...
    public function isAssociatedToShop($shop) { return !isset(self::$unavailable[get_class($this).':'.$this->id.':'.$shop]); }
    public function getFieldByLang($field) { return $this->$field; }
}
...
class Dispatcher
{
    ...
    public static function getInstance() { return new self(); }
    public function getController() { return self::$controller; }
    public function hasKeyword($route, $lang, $keyword, $shop) { return $route === 'product_rule' && $keyword === 'category'; }
    public function createUrl($route, $lang, $params, $force = false, $anchor = '', $shop = null) {
        if ($route === 'product_rule') { return $params['category'].'/'.$params['rewrite'].'-'.$params['id'].'.html'; }
        if ($route === 'category_rule') { return $params['rewrite'].'-'.$params['id'].'/'; }
        if ($route === 'cms_rule') { return 'pages/'.$params['rewrite'].'-'.$params['id'].'.html'; }
        if ($route === 'cms_category_rule') { return 'pages/'.$params['rewrite'].'-'.$params['id'].'/'; }
        if ($route === 'bestsales' || $route === 'newproducts') { return 'index.php?controller='.$route; }
        return $route;
    }
}
...
{
    public function __construct() { $this->allow = 1; }
    ...
    public function getLangLink($id_lang = null, Context $context = null, $id_shop = null) { return ''; }
}
class FixtureSmarty
{
    public $values = array();
